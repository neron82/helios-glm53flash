...
public:
  std::string push(Tokenizer& tk, const std::vector<int>& all_ids) {
    std::string bytes = tk.decode(all_ids, /*keep_special=*/true);
    size_t end = bytes.size();
    while (end > 0 && ((unsigned char)bytes[end - 1] & 0xC0) == 0x80) end--;   // continuations
    if (end > 0) {
      unsigned char lead = (unsigned char)bytes[end - 1];
      size_t need = lead < 0x80 ? 1 : (lead >> 5) == 0x6 ? 2 : (lead >> 4) == 0xE ? 3
                            : (lead >> 3) == 0x1E ? 4 : 1;
      if (bytes.size() - (end - 1) < need) end--;   // incomplete lead sequence
    }
    std::string out = bytes.substr(prev_, end - prev_);
    prev_ = end;
    return out;
  }
  void commit_all() { prev_ = std::string::npos; }
