...
// (the original header declared this with six parameters while kernel_map.cu defines it with
// eight; the extra bszm_in / bszm_out scale k and n for the multi-matrix shape heuristic)
...
// Tilesize / blockdim tables, indexed by shape_idx (index 0 unused). Defined in kernel_map.cu.
extern int exl3_gemm_tilesize_k[];
extern int exl3_gemm_tilesize_n[];
extern int exl3_gemm_blockdim[];
...
// Shape 1 (16x16x128, 6/5 stages) is pruned from the build: select_gemm_shape never
// returns it for CC <= 890 and helios' Hadamard grouping guarantees k % 128 == 0, so
// shape 2 (k % 32, n % 128) is always available. Index 1 stays null; forcing shape 1
// trips the selector assertion.
