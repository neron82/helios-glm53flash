...
{
    HELIOS_ASSERT(K >= 1 && K <= 8, "K must be in [1,8]");
    // Caller responsibility (was TORCH_CHECK_SHAPES(unpacked, 0, packed, 0, 16) and
    // TORCH_CHECK_SIZE(packed, 2, 256 * K / 16)): unpacked has packed_rows * 16 rows,
    // packed holds packed_rows * packed_cols * (256 * K / 16) uint16.

    if (unpacked_cols == 0 || packed_rows == 0)
        return;

    HELIOS_ASSERT(unpacked_cols % 128 == 0, "unpacked N dimension must be divisible by 128");
    HELIOS_ASSERT(n_offset % 128 == 0, "n_offset must be divisible by 128");
    HELIOS_ASSERT(n_offset >= 0, "n_offset must be non-negative");
    HELIOS_ASSERT(n_offset + unpacked_cols <= packed_cols * 16, "reconstruct slice exceeds packed tensor bounds");

    int cols = unpacked_cols / 16;
    int packed_n_offset = n_offset / 16;

    dim3 blockDim(256);
    dim3 gridDim(cols / 8, packed_rows);

    int cbi = K - 1;
    if (mcg) cbi += 8;
    else if (mul1) cbi += 16;

    reconstruct_kernel_instances[cbi]<<<gridDim, blockDim, 0, s>>>
    (
        (half*) unpacked,
...
{
    HELIOS_ASSERT(K >= 1 && K <= 8, "K must be in [1,8]");
    // Caller responsibility (was the TORCH_CHECK_SHAPES / TORCH_CHECK_SIZE / TORCH_CHECK_DTYPE
    // block): unpacked is fp16 with packed_rows * 16 rows, suh fp16 with >= unpacked_rows
    // entries, svh fp16 with >= unpacked_cols entries (already offset by n_offset).

    if (unpacked_cols == 0 || unpacked_rows == 0)
        return;

    HELIOS_ASSERT(unpacked_rows % 128 == 0, "reconstruct_had: K dimension must be divisible by 128");
    HELIOS_ASSERT(unpacked_cols % 128 == 0, "reconstruct_had: N dimension must be divisible by 128");
    HELIOS_ASSERT(n_offset % 128 == 0, "n_offset must be divisible by 128");
    HELIOS_ASSERT(n_offset >= 0, "n_offset must be non-negative");
    HELIOS_ASSERT(n_offset + unpacked_cols <= packed_cols * 16, "reconstruct slice exceeds packed tensor bounds");

    dim3 blockDim(256);
    dim3 gridDim(unpacked_cols / 128, unpacked_rows / 128);

    int cbi = K - 1;
    if (mcg) cbi += 8;
    else if (mul1) cbi += 16;

    reconstruct_had_kernel_instances[cbi]<<<gridDim, RH_THREADS, 0, s>>>
    (
        (half*) unpacked,
        (const uint16_t*) packed,
        (const half*) suh,
...
{
    HELIOS_ASSERT(unpacked_cols == packed_cols * 16, "unpacked and packed have incompatible widths");
    reconstruct_slice(unpacked, packed, packed_rows, packed_cols, unpacked_cols, K, mcg, mul1, 0, s);
}
} // namespace helios::exl3
