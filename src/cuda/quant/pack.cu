...
)
{
    ...
    HELIOS_ASSERT(rows >= 0 && cols >= 0, "rows/cols must be non-negative");
    // Caller responsibility (was TORCH_CHECK_SHAPES / TORCH_CHECK_SIZE on the tensors):
    // packed must hold rows * cols * (256 * K / 16) uint16, unpacked rows * cols * 256 uint16.
    ...
    pack_trellis_kernel_instances[K - 1]<<<gridDim, blockDim, 0, s>>>
    (
        (uint16_t*) packed,
        (const uint16_t*) unpacked
    );
...
)
{
    ...
    unpack_trellis_kernel_instances[K - 1]<<<gridDim, blockDim, 0, s>>>
    (
        (uint16_t*) unpacked,
...
{
    HELIOS_ASSERT(cols >= 0, "cols must be non-negative");
    // unpacked is fp16 with cols * 16 values, packed uint16 with cols values.
    ...
    pack_signs_kernel<<<gridDim, blockDim, 0, s>>>
    (
        (uint16_t*) packed,
        (const uint16_t*) unpacked,
        cols
