...
// Pruned instantiation set: bit widths 2..6 plus 8 (lm_head), see README_PORT.md.
// K = 1 and K = 7 comp units are not compiled; their table rows are null and the
// selectors assert on them.
...
static fp_exl3_gemm_kernel* const tab_gemm_fp32[9][3] =
{
    ...
    { nullptr, nullptr, nullptr }, EXL3_KERNEL_TABLE_ROW(fp32, 2), EXL3_KERNEL_TABLE_ROW(fp32, 3),
    EXL3_KERNEL_TABLE_ROW(fp32, 4), EXL3_KERNEL_TABLE_ROW(fp32, 5), EXL3_KERNEL_TABLE_ROW(fp32, 6),
    { nullptr, nullptr, nullptr }, EXL3_KERNEL_TABLE_ROW(fp32, 8)
};

static fp_exl3_gemm_kernel* const tab_gemm_fp16[9][3] =
{
    { nullptr, nullptr, nullptr },
    { nullptr, nullptr, nullptr }, EXL3_KERNEL_TABLE_ROW(fp16, 2), EXL3_KERNEL_TABLE_ROW(fp16, 3),
    EXL3_KERNEL_TABLE_ROW(fp16, 4), EXL3_KERNEL_TABLE_ROW(fp16, 5), EXL3_KERNEL_TABLE_ROW(fp16, 6),
    { nullptr, nullptr, nullptr }, EXL3_KERNEL_TABLE_ROW(fp16, 8)
};

static fp_exl3_mgemm_kernel* const tab_mgemm_fp32[9][3] =
{
    { nullptr, nullptr, nullptr },
    { nullptr, nullptr, nullptr }, EXL3_MKERNEL_TABLE_ROW(fp32, 2), EXL3_MKERNEL_TABLE_ROW(fp32, 3),
    EXL3_MKERNEL_TABLE_ROW(fp32, 4), EXL3_MKERNEL_TABLE_ROW(fp32, 5), EXL3_MKERNEL_TABLE_ROW(fp32, 6),
    { nullptr, nullptr, nullptr }, EXL3_MKERNEL_TABLE_ROW(fp32, 8)
};

static fp_exl3_mgemm_kernel* const tab_mgemm_fp16[9][3] =
{
    { nullptr, nullptr, nullptr },
    { nullptr, nullptr, nullptr }, EXL3_MKERNEL_TABLE_ROW(fp16, 2), EXL3_MKERNEL_TABLE_ROW(fp16, 3),
    EXL3_MKERNEL_TABLE_ROW(fp16, 4), EXL3_MKERNEL_TABLE_ROW(fp16, 5), EXL3_MKERNEL_TABLE_ROW(fp16, 6),
    { nullptr, nullptr, nullptr }, EXL3_MKERNEL_TABLE_ROW(fp16, 8)
};
...
)
{
    ...
    HELIOS_ASSERT(shape_idx > 0, "exl3_gemm: no compatible kernel");
    ...
    HELIOS_ASSERT(K >= 1 && K <= 8 && cb >= 0 && cb <= 2, "No kernel for GEMM shape");
    fp_exl3_gemm_kernel kernel = (c_fp32 ? tab_gemm_fp32 : tab_gemm_fp16)[K][cb][shape_idx];
    HELIOS_ASSERT(kernel != nullptr, "exl3_gemm: no kernel instance for this bit width / codebook");
    return kernel;
}
...
)
{
    ...
    HELIOS_ASSERT(shape_idx > 0, "exl3_mgemm: no compatible kernel");
    if (out_shape_idx) *out_shape_idx = shape_idx;
    if (out_block_dim) *out_block_dim = exl3_gemm_blockdim[shape_idx];
    ...
    HELIOS_ASSERT(kernel != nullptr, "exl3_mgemm: no kernel instance for this bit width / codebook");
    return kernel;
}
...
fp_exl3_gemm_kernel get_gemm_kernel_ptr(int K, int shape_idx, bool c_fp32, int cb)
{
    HELIOS_ASSERT(K >= 1 && K <= 8 && cb >= 0 && cb <= 2, "No kernel for GEMM shape");
    fp_exl3_gemm_kernel kernel = (c_fp32 ? tab_gemm_fp32 : tab_gemm_fp16)[K][cb][shape_idx];
    HELIOS_ASSERT(kernel != nullptr, "get_gemm_kernel_ptr: no kernel instance for this bit width / codebook");
    return kernel;
}
...
fp_exl3_mgemm_kernel get_mgemm_kernel_ptr(int K, int shape_idx, bool c_fp32, int cb)
{
    HELIOS_ASSERT(K >= 1 && K <= 8 && cb >= 0 && cb <= 2, "No kernel for GEMM shape");
    fp_exl3_mgemm_kernel kernel = (c_fp32 ? tab_mgemm_fp32 : tab_mgemm_fp16)[K][cb][shape_idx];
    HELIOS_ASSERT(kernel != nullptr, "get_mgemm_kernel_ptr: no kernel instance for this bit width / codebook");
    return kernel;
}
